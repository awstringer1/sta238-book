...
This should take you at least an hour or so to do in detail, if not longer.
...
**Remark**: this isn't trivial; for example, [*Benford's Law*](https://en.wikipedia.org/wiki/Benford%27s_law)
says that this is *not* true about the *first* digit in numbers in certain types of financial data.

So, let's look at the last digit of the measurements from the hand sanitizer study.
The data is available from [http://datacolada.org/appendix/74/](http://datacolada.org/appendix/74/) and their `R` code can be found [here](http://datacolada.org/appendix/74/74%20-%20DataColada%20-%20Decoy%20Sanitizer%20-%202018%2008%2023.R). Here we
...
Okay, now consider Figure 1 in the blog post. **They filter out observations ending in $0$ or $5$** 
...
1. Simulate from a continuous uniform distribution with mean and variance equal to the sample
mean and variance (you have to figure out how to do this),

1. Simulate from a discrete uniform distribution with mean and variance equal to the
sample mean and variance,

1. (*Advanced*) Simulate from a discrete uniform distribution as above, but then, chop off the last
digit and replace it with one simulated from a discrete uniform distribution on $\left\{0,\ldots,9\right\}$.

**Exercise**: repeate the simulation, but filter out numbers ending in $0$ or $5$ from
the simulated datasets. Do the results change at all?
...
equals $j$, for $j \in J = \left\{1,2,3,4,6,7,8,9\right\}$ (remember: they filtered out 
measurements ending in $0$ or $5$), and equals $0$ otherwise. So for example,
